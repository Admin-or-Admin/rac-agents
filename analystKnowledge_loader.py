...
KNOWLEDGE_DIR  = os.getenv("ANALYST_KNOWLEDGE_DIR", "analystKnowledge")
...
CHUNK_SIZE     = int(os.getenv("ANALYST_KNOWLEDGE_CHUNK_SIZE", "500"))
CHUNK_OVERLAP  = int(os.getenv("ANALYST_KNOWLEDGE_CHUNK_OVERLAP", "50"))
TOP_K_CHUNKS   = int(os.getenv("ANALYST_KNOWLEDGE_TOP_K", "5"))
...
def _save_cache(chunks: list, embeddings: list):
    ...
    print(f"  [AnalystKnowledge] Cache saved to {CACHE_FILE}")
...
class KnowledgeStore:
    ...
    def load(self):
        ...
        if _cache_is_valid():
            print(f"  [AnalystKnowledge] Cache valid — loading from {CACHE_FILE}")
            try:
                self.chunks, self.embeddings = _load_cache()
                self.loaded = True
                print(f"  [AnalystKnowledge] Loaded {len(self.chunks)} chunks from cache")
                return
            except Exception as e:
                print(f"  [AnalystKnowledge] Cache read failed ({e}) — rebuilding...")

        print(f"  [AnalystKnowledge] Building embedding cache from '{KNOWLEDGE_DIR}'...")
        all_chunks = []
        ...
        for filename in sorted(os.listdir(KNOWLEDGE_DIR)):
            ...
            try:
                ...
                print(f"  [AnalystKnowledge] Read: {filename} ({len(chunks)} chunks)")
            except Exception as e:
                print(f"  [AnalystKnowledge] Failed to load {filename}: {e}")

        if not all_chunks:
            print(f"  [AnalystKnowledge] No files found in '{KNOWLEDGE_DIR}' — skipping RAG")
            self.loaded = True
            return

        print(f"  [AnalystKnowledge] Embedding {len(all_chunks)} chunks with {EMBED_MODEL}...")
        self.chunks = all_chunks
        self.embeddings = _embed_texts([c["text"] for c in all_chunks])
        self.loaded = True
        _save_cache(all_chunks, self.embeddings)
        print(f"  [AnalystKnowledge] Ready — {len(self.chunks)} chunks indexed")
